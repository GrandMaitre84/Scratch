...
// GAME: Game Container pour Phaser
...
function showTab(tab) {
  ...
  switch (tab) {
    ...
    case 'game':
      ...
      if (!phaserGame) {
        setTimeout(() => launchPhaserGame(), 300);
      }
      break;
  }
}
...
/* ─────────────── GAME - Initialisation Phaser plein écran ─────────────── */
function launchPhaserGame() {
  if (phaserGame) return;

  const width = gameContainer.clientWidth || window.innerWidth;
  const height = gameContainer.clientHeight || (window.innerHeight - 64);
